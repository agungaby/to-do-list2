...
export default function TaskLoginView({ email, password, alert, setEmail, setPassword, onSubmit }: Props) {
  ...
  <div className="min-h-screen flex items-center justify-center p-4 relative">
    <div className="bg-black p-8 rounded shadow-md max-w-md w-full outline-4">
      ...
      {/* Alert utama (tengah atas form) */}
      {alert && !alert.toLowerCase().includes('sesi kamu') && (
        <div
    ...
    {alert.toLowerCase().includes('sesi kamu') && (
      <div className="fixed bottom-4 right-4 bg-yellow-500 text-black p-3 rounded shadow-md animate-bounce">
        {alert}
      </div>
    )}
  </div>
